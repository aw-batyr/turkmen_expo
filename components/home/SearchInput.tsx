  ...
  const fetchSearchData = async () => {
    ...
    try {
      const res = await fetch(
        `${baseAPI}search?search=${value}${
          inputStatus !== "all" ? "&filter=" + inputStatus : ""
        }`,
        {
          headers: {
          ...
          <div className="flex flex-col gap-9">
            <div>
              <h3 className="font-bold mb-[18px] text-[16px] leading-[125%]">
                Обувь. Мир кожи-2024. Осень
              </h3>
              <p className="mb-[18px] leading-[130%] text-gray4">
                60-я международная выставка обуви и изделий из кожи Готовые
                изделия —
                обувь женская обувь мужская обувь детская обувь домашняя обувь и обувь для
                отдыха спортивная …
              </p>
              <Link href={""}>Перейти на страницу</Link>
