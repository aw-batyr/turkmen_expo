...
export const FaqSec = () => {
  ...
  const fetchFaq = async () => {
    try {
      const res = await fetch(
        `${baseAPI}faq-headers${currentRadio !== 0 ? '?faq_user_group_id=' + currentRadio : ''}`,
        {
          headers: {
            'Accept-Language': activeLang.localization,
          },
  ...
  return (
    <div className="container flex flex-col items-start pt-[20px] section-mb">
      <BreadCrumbs second={useLang('FAQ', '«Вопросы-ответы»', activeLang.localization)} />
      <Title text={useLang('FAQ', '«Вопросы-ответы»', activeLang.localization)} />
