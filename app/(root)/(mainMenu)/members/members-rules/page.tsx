...
const MembersRules = () => {
  ...
  const [membersRulesData, setMembersRulesData] = useState<ParticipantsDataType>();
  ...
  return (
    <div>
      <LayoutWithSidebar
        title={useLang('Rules for participants', 'Правила для участников', activeLang.localization)}
        ...
        <div className="">
          {membersRulesData
            ? membersRulesData.data.map((item) => <SelectRules key={item.title} {...item} />)
            : null}
        </div>
      </LayoutWithSidebar>
    </div>
